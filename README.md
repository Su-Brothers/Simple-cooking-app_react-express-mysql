...
bcrypt를 사용하여 비밀번호를 암호화하여 데이터베이에 넣어주었다.
```javascript
//user.js 라우트 중 일부
//비밀번호 암호화
...
cloudfront덕분에 react측은 ssl인증서를 발급받을 필요가 없었지만 ec2는 달랐기 때문에 ssl인증서를 받기위해서 도메인을 샀고, 
let's encrypt라는 곳에서 ssl인증서를 무료로 발급 받아 자동갱신을 해주었다.

# learned...

1. 클라이언트와 서버측의 통신 방법에 대해 많이 배웠다.
